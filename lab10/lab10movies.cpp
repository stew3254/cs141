...
int main()
{
	ifstream inputHandler;		//file handler for input file
	string movieNameFromFile;	//store movie name from file
...
void showBeforeYear(node* currentPtr, int userYear)
{
	...
	while (currentPtr)
	{
		...
		{
			found++;
			cout << currentPtr->aMovie.movieName << endl;
		}
		currentPtr = currentPtr->prevPtr;
	}
	if (found == 0)
	{
		cout << "No movies found before " << userYear << endl;
	}
